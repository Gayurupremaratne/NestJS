...
describe('UserTrailTrackingSummaryController', () => {
  let userTrailTrackingSummaryController: UserTrailTrackingSummaryController;
  ...
  beforeAll(async () => {
    ...
    userTrailTrackingSummaryController = module.get<UserTrailTrackingSummaryController>(
      UserTrailTrackingSummaryController,
    );
  ...
  it('should be defined', () => {
    expect(userTrailTrackingSummaryController).toBeDefined();
  });
  ...
  describe('Should get user tracking summary', () => {
    it('should get user tracking summary details', async () => {
      ...
      const response = await userTrailTrackingSummaryController.getUserTrailSummary({
        userId: reqUser.sub,
      });

      expect(response.data).toStrictEqual(responseDetails);
    });

    it('should get logged in user tracking summary details', async () => {
      const responseDetails = [
        {
          totalCompletedStages: 0,
          totalDuration: userTrailUpdatePayload.totalTime,
      ...
      const response = await userTrailTrackingSummaryController.getUserTrailSummaryByUser(reqUser);

      expect(response.data).toStrictEqual(responseDetails);
    });
  });

  describe('Should get user tracking completion summary', () => {
    it('should get user tracking completion summary details', async () => {
      const today = new Date();
      const month = today.getMonth() + 1;
      const year = today.getFullYear();
      const response = await userTrailTrackingSummaryController.getAllUsersTrailTrackingSummary({
        stageId: stageDto.id,
        month,
        year,
      });
  ...
  describe('Should get user tracking summary of all stages', () => {
    it('Should get user tracking summary of all stages', async () => {
      const response = await userTrailTrackingSummaryController.getUserTrailTrackingByStage(
        { userId: reqUser.sub },
        { pageNumber: 1 },
      );

      expect(response.data.data[0].stageId).toStrictEqual(stageDto.id);
    });
  });

  describe('Should get user tracking summary by passId', () => {
    it('Should get user tracking summary of all stages', async () => {
      const response = await userTrailTrackingSummaryController.getUserTrailTrackingByPassId(
        { passesId: passDto.id },
        reqUser,
      );

      expect(response.data.passesId).toStrictEqual(passDto.id);
    });
  });
